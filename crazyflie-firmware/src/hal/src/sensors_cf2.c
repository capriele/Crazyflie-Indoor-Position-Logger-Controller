...
static float magn_ellipsoid_center_x = 0.435398053830192;
static float magn_ellipsoid_center_y = -1.373302096948014;
static float magn_ellipsoid_center_z = -0.030116289556647;

static float magn_ellipsoid_transform_0_x = 0.994909702132739;
static float magn_ellipsoid_transform_0_y = 0.013992398894093;
static float magn_ellipsoid_transform_0_z = 0.007102380825420;
static float magn_ellipsoid_transform_1_x = 0.013992398894093;
static float magn_ellipsoid_transform_1_y = 0.954267519233219;
static float magn_ellipsoid_transform_1_z = -0.000493453927448;
static float magn_ellipsoid_transform_2_x = 0.007102380825420;
static float magn_ellipsoid_transform_2_y = -0.000493453927448;
static float magn_ellipsoid_transform_2_z = 0.940275678598872;
void processMagnetometerMeasurements(const uint8_t *buffer)
{
  if (buffer[0] & (1 << AK8963_ST1_DRDY_BIT)) {
    ...
    float mx_tmp = (float)headingx / MAG_GAUSS_PER_LSB - magn_ellipsoid_center_x;
    float my_tmp = (float)headingy / MAG_GAUSS_PER_LSB - magn_ellipsoid_center_y;
    float mz_tmp = (float)headingz / MAG_GAUSS_PER_LSB - magn_ellipsoid_center_z;
    sensors.mag.y = magn_ellipsoid_transform_0_x * mx_tmp + magn_ellipsoid_transform_0_y * my_tmp + magn_ellipsoid_transform_0_z * mz_tmp;
    sensors.mag.x = magn_ellipsoid_transform_1_x * mx_tmp + magn_ellipsoid_transform_1_y * my_tmp + magn_ellipsoid_transform_1_z * mz_tmp;
    sensors.mag.z = -(magn_ellipsoid_transform_2_x * mx_tmp + magn_ellipsoid_transform_2_y * my_tmp + magn_ellipsoid_transform_2_z * mz_tmp);
    sensors.mag.x -= 0.868354634724386f + 0.060f;// - 0.44f;
    sensors.mag.y -= 1.216750112723695f + 0.200f;// - 0.44f;
    sensors.mag.z += 0.260246078144149f;// - 0.210f;
  }
}
